...
train_model.py
==============
AI Traffic Operations Platform — Model Training Script
------------------------------------------------------
Dataset : Astram event data (anonymized)
Models  : Two Random Forest classifiers
          1. severity_model   → predicts severity label   (Low / Medium / High / Critical)
          2. priority_model   → predicts priority label   (Low / High)

Labels are derived from the deterministic rule engine defined in the project
specification so that the ML models learn to replicate — and generalise — the
same business logic from raw event features alone.

Usage
-----
    python train_model.py --data path/to/events.csv --out models/

Outputs (written to --out directory)
--------------------------------------
    severity_model.pkl          Random Forest for severity
    priority_model.pkl          Random Forest for priority
    label_encoders.pkl          Dict of fitted LabelEncoders for categorical features
    feature_columns.json        Ordered list of feature names (for inference)
    training_report.txt         Accuracy, classification report, feature importance
"""
...
def main() -> None:
    ...
    # ── Train models ──────────────────────────────────────────────────────────
    print("\n[4/6] Training models …")
    severity_model, sev_report = train_model(
        X, y_severity, SEVERITY_LABELS, "Severity Classifier", args.seed
    )
    priority_model, prio_report = train_model(
